/*
  # Fix provision_tenant function - use correct subscription status

  1. Changes
    - Change subscription status from 'trialing' to 'active'
    - The subscription_status enum only has: active, past_due, cancelled
*/

-- Drop existing function
DROP FUNCTION IF EXISTS provision_tenant(uuid, text, text, text, text, text, text, text);

-- Recreate with correct subscription status
...
BEGIN
  ...
  -- Insert tenant_subscriptions record with 'active' status
