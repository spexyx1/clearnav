/*
  # Fix provision_tenant function to match all actual schemas

  1. Changes
    - Update user_roles insert to use: id, user_id, email, role_category, role_detail, tenant_id, status, metadata
    - Update tenant_settings insert to use: tenant_id, branding, features, notifications, integrations
    - Update tenant_subscriptions insert to use: id, tenant_id, plan_id, status, current_period_start, current_period_end
    - Store additional data in metadata/branding jsonb fields

  2. Notes
    - Drops and recreates the function with correct schema for all tables
*/

-- Drop existing function
DROP FUNCTION IF EXISTS provision_tenant(uuid, text, text, text, text, text, text, text);

-- Recreate the secure tenant provisioning function with all correct schemas
...
BEGIN
  ...
  -- Insert user_roles record (tenant_admin) with correct columns
  ...
  -- Insert tenant_settings record with correct columns
  ...
  -- Insert tenant_subscriptions record with correct columns
