/*
  # Fix provision_tenant function to match actual schema

  1. Changes
    - Update function to use correct platform_tenants columns
    - Remove references to non-existent columns
    - Use actual schema: id, slug, name, status, database_type, is_self_service, contact_email, contact_name, signup_completed_at

  2. Notes
    - Drops and recreates the function with correct schema
*/

-- Drop existing function
DROP FUNCTION IF EXISTS provision_tenant(uuid, text, text, text, text, text, text, text);

-- Recreate the secure tenant provisioning function with correct schema
...
BEGIN
  ...
  -- Insert platform_tenants record with correct schema
