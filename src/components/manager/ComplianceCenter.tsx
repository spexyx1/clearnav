...
type ComplianceTab = 'kyc' | 'documents' | 'accreditation';
...
export default function ComplianceCenter() {
  ...
  const kycApproved = kycRecords.filter(k => k.didit_session_status === 'Approved').length;
  const kycPending = kycRecords.filter(k => !k.didit_session_status || k.didit_session_status === 'Not Started' || k.didit_session_status === 'Abandoned').length;
  const verifiedAccred = accreditations.filter(a => a.verified_accredited).length;

  const tabs: { id: ComplianceTab; label: string; badge?: number }[] = [
    { id: 'kyc', label: 'KYC / AML Screening', badge: kycPending > 0 ? kycPending : undefined },
    { id: 'documents', label: 'Documents', badge: pendingDocs > 0 ? pendingDocs : undefined },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400">KYC/AML screening, document verification, and accreditation tracking</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-slate-900/50 border border-slate-800/50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-2xl font-bold text-white">{kycApproved}</span>
          </div>
          <div className="text-sm text-slate-400">KYC Approved</div>
        </div>
        <div className="bg-slate-900/50 border border-slate-800/50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
        ...
        <div className="bg-slate-900/50 border border-slate-800/50 rounded-lg p-6">
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {accreditations.length === 0 ? (
              ...
              accreditations.slice(0, 20).map((accred) => (
