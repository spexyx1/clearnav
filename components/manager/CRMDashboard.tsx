...
export default function CRMDashboard({ onNavigate }: CRMDashboardProps) {
  ...
  const loadDashboardData = async () => {
    ...
    const filteredByInvestment = (contactsRes.data || []).filter(c => c.estimated_investment_amount);
    const avgInvestment = filteredByInvestment.reduce((sum, c) => sum + (c.estimated_investment_amount ?? 0), 0) / (filteredByInvestment.length || 1);
