...
import { TabType } from './ManagerSidebar';
...
export default function CRMDashboard({ onNavigate }: CRMDashboardProps) {
  ...
  const loadDashboardData = async () => {
    ...
    const avgInvestment = (contactsRes.data?.filter(c => c.estimated_investment_amount).reduce((sum, c) => sum + (c.estimated_investment_amount ?? 0), 0) ?? 0) / (contactsRes.data?.filter(c => c.estimated_investment_amount).length || 1);
