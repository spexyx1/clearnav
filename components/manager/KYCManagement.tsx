    ...
    const createSession = async () => {
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/didit-create-session`,
          {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    ...
    const initSdk = async () => {
      try {
        ...
        if (sdk) sdk.onStateChange = (state: string) => {
          setSdkState(state);
        };

        if (sdk) sdk.onComplete = (result: unknown) => {
